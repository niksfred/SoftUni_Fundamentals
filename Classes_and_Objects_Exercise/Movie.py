class Movie:
    ...
    def watch(self):
        if not self.name in Movie.__watched__movies: 
            self.watched = True
            Movie.__watched__movies.append(self.name)

    def __repr__(self):
        return f"Movie name: {self.name}; Movie director: {self.director}. Total watched movies: {len(Movie.__watched__movies)}"
